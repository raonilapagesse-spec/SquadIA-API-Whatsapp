...
const RATE_LIMIT_WINDOW_MS = Number(process.env.BRIDGE_RATE_LIMIT_WINDOW_MS || 60_000);
const RATE_LIMIT_MAX = Number(process.env.BRIDGE_RATE_LIMIT_MAX || 120);
const WEBHOOK_ALLOWED_HOSTS = new Set(
  String(process.env.BRIDGE_WEBHOOK_ALLOWED_HOSTS || "")
    .split(",")
...
let knownSessionCount = fs.existsSync(DATA_DIR) ? fs.readdirSync(DATA_DIR).length : 0;
...
function sessionDir(ref) {
  ...
  return path.join(DATA_DIR, safeRef);
}
...
function saveMeta(ref, patch) {
  ...
  knownSessionCount = fs.existsSync(DATA_DIR) ? fs.readdirSync(DATA_DIR).length : knownSessionCount;
  return next;
}
...
function normalizeWebhookUrl(raw) {
  ...
  try {
    ...
    if (WEBHOOK_ALLOWED_HOSTS.size === 0) return null;
...
async function postWebhook(webhookUrl, event) {
  const safeWebhookUrl = normalizeWebhookUrl(webhookUrl);
...
app.get("/health", (_req, res) => {
  ...
  res.json({
    ...
    known: knownSessionCount,
  });
});

/** Pareamento explícito (compatível com o app atual). */
app.post("/sessions", async (req, res) => {
  const { externalId, phone, webhookUrl: rawWebhookUrl } = req.body || {};
...
app.post("/sessions/:ref/pair", async (req, res) => {
  ...
  if (!ref) return res.status(400).json({ error: "ref inválida" });
  const meta = loadMeta(ref) || {};
  const externalId = req.body?.externalId ?? meta.externalId;
  const phone = req.body?.phone ?? meta.phone;
  const webhookUrl = normalizeWebhookUrl(req.body?.webhookUrl ?? meta.webhookUrl);
  if (!externalId || !phone || !webhookUrl) {
...
app.delete("/sessions/:ref", async (req, res) => {
  ...
  knownSessionCount = fs.existsSync(DATA_DIR) ? fs.readdirSync(DATA_DIR).length : 0;
  res.json({ ok: true });
});
